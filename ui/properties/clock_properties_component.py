...
class ClockPropertiesComponent(BasePropertiesComponent):
    ...
    def init_ui(self):
        ...
        self.clock_coords_x.setMinimumWidth(60)
        self.clock_coords_x.setText("0")
        coords_comma = QLabel(",")
        self.clock_coords_y = QLineEdit()
        self.clock_coords_y.setPlaceholderText("0")
        self.clock_coords_y.setMinimumWidth(60)
        self.clock_coords_y.setText("0")
        coords_layout.addWidget(coords_label)
        coords_layout.addWidget(self.clock_coords_x)
        coords_layout.addWidget(coords_comma)
        coords_layout.addWidget(self.clock_coords_y)
        ...
        self.clock_dims_width.setMinimumWidth(60)
        self.clock_dims_width.setText("1920")
        dims_comma = QLabel(",")
        self.clock_dims_height = QLineEdit()
        self.clock_dims_height.setPlaceholderText("1080")
        self.clock_dims_height.setMinimumWidth(60)
        self.clock_dims_height.setText("1080")
        dims_layout.addWidget(dims_label)
        dims_layout.addWidget(self.clock_dims_width)
        dims_layout.addWidget(dims_comma)
        dims_layout.addWidget(self.clock_dims_height)
        ...
        self.clock_frame_checkbox = QCheckBox("Frame")
        self.clock_frame_checkbox.toggled.connect(self._on_frame_enabled_changed)
        frame_checkbox_layout.addWidget(self.clock_frame_checkbox)
        ...
        self.clock_frame_border_combo = QComboBox()
        borders = self.get_available_borders()
        if borders:
            self.clock_frame_border_combo.addItems(["---"] + borders)
        else:
            self.clock_frame_border_combo.addItems(["---", "000", "001", "002", "003"])
        self.clock_frame_border_combo.setCurrentText("---")
        self.clock_frame_border_combo.setEnabled(False)
        self.clock_frame_border_combo.currentTextChanged.connect(self._on_frame_border_changed)
        border_layout.addWidget(self.clock_frame_border_combo, stretch=1)
        frame_group_layout.addLayout(border_layout)
        
        # Effect selection
        effect_layout = QHBoxLayout()
        effect_layout.addWidget(QLabel("Effect:"))
        self.clock_frame_effect_combo = QComboBox()
        self.clock_frame_effect_combo.addItems(["static", "rotate", "twinkle"])
        self.clock_frame_effect_combo.setCurrentText("static")
        self.clock_frame_effect_combo.setEnabled(False)
        self.clock_frame_effect_combo.currentTextChanged.connect(self._on_frame_effect_changed)
        effect_layout.addWidget(self.clock_frame_effect_combo, stretch=1)
        frame_group_layout.addLayout(effect_layout)
        
        # Speed selection
        speed_layout = QHBoxLayout()
        speed_layout.addWidget(QLabel("Speed:"))
        self.clock_frame_speed_combo = QComboBox()
        self.clock_frame_speed_combo.addItems(["slow", "in", "fast"])
        self.clock_frame_speed_combo.setCurrentText("in")
        self.clock_frame_speed_combo.setEnabled(False)
        self.clock_frame_speed_combo.currentTextChanged.connect(self._on_frame_speed_changed)
        speed_layout.addWidget(self.clock_frame_speed_combo, stretch=1)
    ...
    def update_properties(self):
        ...
        self.clock_frame_checkbox.blockSignals(True)
        self.clock_frame_checkbox.setChecked(frame_enabled)
        self.clock_frame_checkbox.setEnabled(True)  # Ensure checkbox is enabled
        self.clock_frame_checkbox.blockSignals(False)
        
        self.clock_frame_border_combo.setEnabled(frame_enabled)
        self.clock_frame_effect_combo.setEnabled(frame_enabled)
        self.clock_frame_speed_combo.setEnabled(frame_enabled)
        
        border = frame_props.get("border", "---") if isinstance(frame_props, dict) else "---"
        border_index = self.clock_frame_border_combo.findText(border)
        if border_index >= 0:
            self.clock_frame_border_combo.setCurrentIndex(border_index)
        else:
            self.clock_frame_border_combo.setCurrentIndex(0)
        
        effect = frame_props.get("effect", "static") if isinstance(frame_props, dict) else "static"
        effect_index = self.clock_frame_effect_combo.findText(effect)
        if effect_index >= 0:
            self.clock_frame_effect_combo.setCurrentIndex(effect_index)
        else:
            self.clock_frame_effect_combo.setCurrentIndex(0)
        
        speed = frame_props.get("speed", "in") if isinstance(frame_props, dict) else "in"
        speed_index = self.clock_frame_speed_combo.findText(speed)
        if speed_index >= 0:
            self.clock_frame_speed_combo.setCurrentIndex(speed_index)
        else:
            self.clock_frame_speed_combo.setCurrentIndex(1)
    ...
    def _on_frame_enabled_changed(self, enabled: bool):
        ...
        if not self.current_element or not self.current_program:
            ...
            self.clock_frame_checkbox.blockSignals(True)
            self.clock_frame_checkbox.setEnabled(False)
            self.clock_frame_checkbox.setChecked(False)
            self.clock_frame_checkbox.blockSignals(False)
            return
        
        # Ensure checkbox is enabled when we have element/program
        self.clock_frame_checkbox.setEnabled(True)
        self.clock_frame_border_combo.setEnabled(enabled)
        self.clock_frame_effect_combo.setEnabled(enabled)
        self.clock_frame_speed_combo.setEnabled(enabled)
        if "properties" not in self.current_element:
            self.current_element["properties"] = {}
        ...
        self.current_program.modified = datetime.now().isoformat()
        self.property_changed.emit("clock_frame_enabled", enabled)
        self._trigger_autosave()
    ...
    def _on_frame_border_changed(self, border: str):
        ...
        self.current_program.modified = datetime.now().isoformat()
        self.property_changed.emit("clock_frame_border", border)
        self._trigger_autosave()
    ...
    def _on_frame_effect_changed(self, effect: str):
        ...
        self.current_program.modified = datetime.now().isoformat()
        self.property_changed.emit("clock_frame_effect", effect)
        self._trigger_autosave()
    ...
    def _on_frame_speed_changed(self, speed: str):
        ...
        self.current_program.modified = datetime.now().isoformat()
        self.property_changed.emit("clock_frame_speed", speed)
        self._trigger_autosave()
