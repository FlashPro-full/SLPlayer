...
try:
    ...
    from sdk.deviceTask.PeriodicTask import PeriodicTask
...
class HuiduSDK:
    ...
    def is_card_online(self, ip_address: str, send_type: int = 0) -> bool:
        ...
        if not self._initialized:
            ...
            try:
                ...
                logger.debug(f"Temporary SDK initialization failed for {ip_clean}: {e}")
                return False
    ...
    def create_program(self, program_node: ProgramNode, device_ids: List[str] = None) -> Dict:
    ...
    def update_program(self, program_node: ProgramNode, device_ids: List[str] = None) -> Dict:
        if not self._initialized or not self.program:
            return {"message": "error", "data": "SDK not initialized"}
    ...
    def replace_program(self, program_node: ProgramNode, device_ids: List[str] = None) -> Dict:
        if not self._initialized or not self.program:
            return {"message": "error", "data": "SDK not initialized"}
    ...
    def remove_program(self, program_id: str, device_ids: List[str] = None) -> Dict:
        if not self._initialized or not self.program:
            return {"message": "error", "data": "SDK not initialized"}
    ...
    def set_device_property(self, properties: Dict, device_ids: List[str] = None) -> Dict:
        if not self._initialized or not self.device:
            return {"message": "error", "data": "SDK not initialized"}
    ...
    def get_device_status(self, device_ids: List[str] = None) -> Dict:
        if not self._initialized or not self.device:
            return {"message": "error", "data": "SDK not initialized"}
    ...
    def set_luminance(self, brightness: int, device_ids: List[str] = None) -> Dict:
        return self.set_device_property({"luminance": str(brightness)}, device_ids)
    
    def set_time_switch(self, tasks: Dict, device_ids: List[str] = None) -> Dict:
        if not self._initialized:
            return {"message": "error", "data": "SDK not initialized"}
    ...
    def set_ip(self, ip_config: Dict, device_ids: List[str] = None) -> Dict:
        properties = {}
        if "ip" in ip_config:
