"""
NovaStar controller protocol handler
"""
import json
...
class NovaStarController(BaseController):
    """NovaStar LED controller implementation"""
    ...
    def get_device_info(self) -> Optional[Dict]:
        ...
        try:
            # NovaStar protocol: Send device info request
    ...
    def upload_program(self, program_data: Dict, file_path: str = None) -> bool:
        """Upload program to NovaStar controller"""
    ...
    def download_program(self, program_id: str = None) -> Optional[Dict]:
        """Download program from NovaStar controller"""
        if not self.socket or self.status != ConnectionStatus.CONNECTED:
            return None
    ...
    def _parse_info_response(self, data: bytes) -> Optional[Dict]:
        ...
        # Simplified - actual implementation would parse binary protocol
        # For license system, controller_id should be extracted from device info
        # Format: "NS-M30-00123456" or similar
        controller_id = f"NS-{self.ip_address.replace('.', '-')}"  # Placeholder - should come from device
        return {
    ...
    def _build_upload_command(self, program_data: bytes) -> bytes:
        ...
        # Simplified - actual protocol would have specific structure
    ...
    def _parse_list_response(self, data: bytes) -> List[Dict]:
        ...
        # Simplified - would parse actual protocol
        return []
    ...
    def _convert_program_to_bytes(self, program_data: Dict) -> bytes:
        ...
        # Convert to JSON for now - actual format may be binary
